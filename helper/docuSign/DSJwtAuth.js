...
'use strict'
const eg008 = require('../../controllers/eg008CreateTemplate')
const eg009 = require('../../controllers/eg009UseTemplate')
...
const moment = require('moment'),
  ...
  dsConfig = require('../../dsconfig/index.js').config,
...
DsJwtAuth.prototype.checkToken = function _checkToken (
  ...
  let noToken = !this.accessToken || !this._tokenExpiration,
    ...
    needToken =
      noToken ||
      moment(this._tokenExpiration)
        .subtract(bufferMin, 'm')
        .isBefore(now)
  if (this._debug) {
    if (noToken) {
 ...
 */
DsJwtAuth.prototype.getToken = async function _getToken (dsClientId, impersonatedUserGuid) {
  // Data used
  // dsConfig.dsClientId
 ...
 * @function
 */
 DsJwtAuth.prototype.createTemplate = function (req, res, next) {
  this.internalLogout(req, res)
  req.session.authMethod = 'jwt-auth'
  const log = async () => {
    const auth = await this.getToken(req.template.dsIntegrationId, req.template.dsUserId)
        ...
        if (body.error && body.error === 'consent_required') {
          let consent_scopes = this.scopes + ' impersonation',
            consent_url =
              ...
              `scope=${consent_scopes}&client_id=${req.template.dsIntegrationId}&` +
 ...
 * @function
 */
 DsJwtAuth.prototype.createEnvelope = function (req, res, next) {
  this.internalLogout(req, res)
  req.session.authMethod = 'jwt-auth'
  const log = async () => {
    const auth = await this.getToken(req.signer.dsIntegrationId, req.signer.dsUserId)
    const user = await this.getUserInfo()
    req.session.accountId = user.accountId
    req.session.accountName = user.accountName
    req.session.basePath = user.basePath
      ...
      if (body) {
        ...
        if (body.error && body.error === 'consent_required') {
          let consent_scopes = this.scopes + ' impersonation',
            consent_url =
              `${dsConfig.dsOauthServer}/oauth/auth?response_type=code&` +
              `scope=${consent_scopes}&client_id=${req.signer.dsIntegrationId}&` +
              `redirect_uri=${dsConfig.appUrl}/ds/callback`
          res.redirect(consent_url)
        } else {
          // Consent has been granted. Show status code for DocuSign API error
